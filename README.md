...
- **レシート撮影による自動入力**: codex execでレシートを自動認識
- **AI自動分類**: codex execが出費を適切なカテゴリに自動分類
- **手入力サポート**: 電卓機能付きの簡単入力
...
1. **バックグラウンドタスク処理**: codex execによるレシート認識と分類処理は、それぞれ数秒～数十秒かかる場合があります。これらを同期的に処理するとHTTPリクエストがタイムアウトする可能性があります。
